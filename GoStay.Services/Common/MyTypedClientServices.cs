...
namespace GoStay.Service
{
    ...
    public class MyTypedClientServices : IMyTypedClientServices
    {
        ...
        public UploadImagesResponse PostImgAndGetData(List<IFormFile> files, int width, string? Obj_Id, int type)
        {
            ...
            response = httpClient.PostAsync("https://cdn.realtech.com.vn/api/UploadFile/UploadFile" + $"?width={width}"
                                                        + $"&Obj_Id={Obj_Id}" + $"&type={type}", content).Result;
        ...
        public UploadImagesResponse PostVideoAndGetData(List<IFormFile> files,string title, int type, int IdUser)
        {
            ...
            response = httpClient.PostAsync("https://cdn.realtech.com.vn/api/UploadFile/UploadVideo" + $"?title={title}"
                                                        + $"&IdUser={IdUser}" + $"&type={type}", content).Result;
